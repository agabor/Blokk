...
// returns sequence of squares detected on the image.
// the sequence is stored in the specified memory storage
void findSquares( const Mat& image, vector<vector<Point> >& squares, float min_area = 1000.0f )
{
    squares.clear();
...
Mat getSubImage(const vector<Point> *rect, const Mat *image, float mul, float area_mul = 1)
{
    RotatedRect r = minAreaRect(Mat(*rect));
    r.center.x *= mul;
    r.center.y *= mul;
    r.size.width *= mul * area_mul;
    r.size.height *= mul * area_mul;
...
int main(int /*argc*/, char** /*argv*/)
{
    ...
    for( int i = 0; names[i] != 0; i++ )
    {
        ...
        int j = 1;
        cv::Mat b = getSubImage(&squares[j], &image, 5, 1.1);
        vector<vector<Point> > local;
        findSquares(b, local, 100);
        makeRectListUniq(&local);

        if (local.size() != 0)
        {
        cv::Mat blokk = getSubImage(&local[0], &b, 1); 

        //for(int j = 0; j < squares.size(); ++j)
        {
            const char c[2] = {'0' + j, 0};
            namedWindow( string("blokk") + string(c), 1 );
            imshow(string("blokk") + string(c), blokk);
        }
        ...
        p.xres = 96;
        p.yres = 96;
        string s("blokk image");
        p.informat = IFF_DEFAULT;
        p.text = const_cast<char*>(s.c_str());
        p.colormap = 0;
        //!
        p.data = new l_uint32[p.w * p.h];

        for (int idx = 0; idx < p.h * p.w ; ++idx)
        {
            p.data[idx] = blokk.data[idx * 3];
            p.data[idx] += p.data[idx] << 8;
            p.data[idx] += p.data[idx] << 16;
        }
        
        //api.ProcessPage(&p, 0, "", "", 0, &text_out);
        api.SetImage(&p);

        
        std::cout << std::string(api.GetUTF8Text()) << std::endl;
        }
        else 
